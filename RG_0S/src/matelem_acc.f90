...
!Fixed maximum pseudoparticle count (matches Glob_AllowedNumOfPseudoParticles)
...
real(wp), intent(in)  :: piraised3n2
real(wp), intent(in)  :: MassMatrix(n,n)
real(wp), intent(in)  :: charge(n), charge0
real(wp), intent(in)  :: attr_scale, rep_scale, rep_plus, rep_minus
real(wp), intent(out) :: Hkl, Skl

!Local fixed-size scratch (so the routine needs no heap on the device)
real(wp) :: Lk(ACC_NN,ACC_NN), Ll(ACC_NN,ACC_NN)
real(wp) :: Ak(ACC_NN,ACC_NN), tAl(ACC_NN,ACC_NN), tAkl(ACC_NN,ACC_NN)
real(wp) :: W1(ACC_NN,ACC_NN), W2(ACC_NN,ACC_NN)
real(wp) :: inv_tAkl(ACC_NN,ACC_NN), inv_tAkltAlM(ACC_NN,ACC_NN)
real(wp) :: temp1, temp3, temp4, temp5
real(wp) :: det_tAkl, Tkl, Vkl, cprod
integer     :: i, j, k, indx

...
They read files `cf.dat` and `dens.dat`, respectively, and create plots in the `png` format for all distributions. Two plots are created for each 1D distribution: the function itself (e.g. $g_{12}(r)$, $\rho_1(r)$) and the corresponding radial probability density (e.g. $4 \pi r^2 g_{12}(r)$, $4 \pi r^2 \rho_1(r)$). The resulting `png` files generated by gnuplot for the considered case of Li atom are provided.  

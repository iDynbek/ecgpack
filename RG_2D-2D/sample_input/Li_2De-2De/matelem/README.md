...
The sample files `wf_state0.txt` and `wf_state1.txt` contain the wave functions of the lowest and second-lowest lithium $^2D^{e}$ states, respectively.

These files are extracted from the general-format input files using the `RG_2D` code.
...
$$
\langle {}^{X}D_{J} | \mathcal{H} | {}^{X}D_{J} \rangle.
$$
...
$$
\langle {}^{2}D_{J=5/2} | \mathcal{H} | {}^{2}D_{J=5/2} \rangle.
$$

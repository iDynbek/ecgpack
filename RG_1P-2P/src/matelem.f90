...
!==============================================================================================
    ...
    REAL(dprec)              :: tvk(Glob_np)                ! Transformed bra vector: Pk' * e_{mk}
    REAL(dprec)              :: tvl(Glob_np), twl(Glob_np)  ! Transformed ket vectors: Pl' * e_{ml_1}, Pl' * e_{ml_2}

    REAL(dprec)              :: tvk_inv_tAkl(Glob_np)       ! tvk' * inv_tAkl (row vector)
    REAL(dprec)              :: inv_tAkl_twl(Glob_np)       ! inv_tAkl * twl   (column vector)
    REAL(dprec)              :: inv_tAkl_tvl(Glob_np)       ! inv_tAkl * tvl   (column vector)
    REAL(dprec)              :: det_Lk, det_Ll, det_tAkl
    REAL(dprec)              :: temp01, temp02, temp03, temp1, temp2
    REAL(dprec)              :: charge_mass0

    REAL(dprec)              :: tgamma1, tgamma5, tgamma2, tgamma6     ! Length-gauge tilde gammas
    REAL(dprec)              :: tgamma2_v, tgamma6_v                   ! Velocity-gauge tilde gammas (u_i-based)
    REAL(dprec)              :: gamma1, gamma2, gamma5, gamma6         ! Normalization gammas (P^e norm)

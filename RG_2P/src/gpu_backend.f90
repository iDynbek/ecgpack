module gpu_backend
!The complete GPU backend for RG_2P -- both the high-level orchestration AND the
...
!
!Ported from the RG_0S backend. RG_2P differences handled here:
...
!    MatrixElementsHS_RG_2P, which unpacks L and forms A per (pair x term) call
...
!      SHARED MatrixElementsHS_RG_2P device routine in matelem.f90, so the physics
...
!      code cannot read host module globals.
  ...
  use matelem,   only: MatrixElementsHS_RG_2P
  use cudafor
  use cublas
  use cusolverDn
  implicit none
  ...
  integer, parameter :: CUF_BLK = 128       !threads/block. The term loop STRIDES (see the kernels), so
                                            ...
                                            !MatrixElementsHS_RG_2P is register-heavy on device;
...
contains
  ...
  !terms. Each thread calls the shared MatrixElementsHS_RG_2P (grad=false) for
  !its terms.
    ...
    if (determ /= 0) then
      ...
      do j = threadIdx%x, nterms, blockDim%x
        if (mod(qbase+j, nprocs) == procid) then
          call MatrixElementsHS_RG_2P(n, np, im(k0), im(l0), imm(k0), imm(l0), &
  ...
  !Each thread calls the shared MatrixElementsHS_RG_2P (grad_k=true, grad_l=flag)

...
contains
subroutine MatrixElementsLP(m_k, m_l, mm_k, mm_l, vechLk, vechLl, P, &
...
end subroutine MatrixElementsLP
...
real(dprec)   tbltbk(nn,nn),tvltvk(nn,nn),tbltvk(nn,nn),tvltbk(nn,nn)
...
real(dprec)   ::  tvk_r(nn),tvl_r(nn),tbk_r(nn),tbl_r(nn)
real(dprec)   ::  EMatr(nn,nn), KMatr(nn,nn), DMatr(nn,nn), FMatr(nn,nn), GMatr(nn,nn)
...
tvk_r = real(tvk, kind=dprec)
tvl_r = real(tvl, kind=dprec)
tbk_r = real(tbk, kind=dprec)
tbl_r = real(tbl, kind=dprec)
...
function ME_KDFG(rindexI,rindexJ,KK,DD,FF,GG,tAk,tAl,inv_tAkl,det_tAkl,tvk,tvl,twk,twl)
  ...
  integer,parameter :: nn=Glob_MaxAllowedNumOfPseudoParticles
  !Arguments:
  real(dprec)   KK(nn,nn), DD(nn,nn), FF(nn,nn), GG(nn,nn), &
  tAl(nn,nn), tAk(nn,nn), inv_tAkl(nn,nn), det_tAkl
  real(dprec)       tvk(nn),tvl(nn),twk(nn),twl(nn)
  integer       rindexI,rindexJ
  
  !Local variables: 
  integer     ::  i,j,k,n
  real(dprec) ::  KG(nn,nn), KGCl(nn,nn), CkD(nn,nn),CkDF(nn,nn), CkDFCl(nn,nn), &
  ...
  real(dprec) ::  KGCl_s(nn,nn), CkDFCl_s(nn,nn), KGCltFtDCk_s(nn,nn), &
  tGtKDF_s(nn,nn), KGClDFCl_s(nn,nn), &
  KGCltFtDCl_s(nn,nn), ClDFCl_s(nn,nn)
  real(dprec) ::  CkDFvl(nn), CkDFwl(nn), KGvl(nn), KGwl(nn), &
  ...
  real(dprec) ::  term1, term2, term3, term4, term5, term6, term7, term8, &
  ...
  real(dprec) ::  vkDFvl, vkDFwl, wkDFvl, wkDFwl, trDFCl
  real(dprec) :: SIGN
...
function ME_over_rij_dXd(p,q,X,tAl,inv_tAkl,det_tAkl,tvk,tvl,twk,twl)
  ...
  !Local variables:
  integer     :: c,s,n,k,i,j
  real(dprec) :: ClX(nn,nn), ClXCl(nn,nn), ClXCl_s(nn,nn)
  real(dprec) :: commonFactor, gamma, temp, temp1, temp2, temp3
  real(dprec) :: ClXvl(nn), ClXwl(nn)
  real(dprec) :: trXCl
  real(dprec) :: RQ, RVl, RWl, Rtr 
...
function ME_over_rij_WkWl_real(p,q,inv_tAkl,det_tAkl,twk,twl)
  real(dprec)  ME_over_rij_WkWl_real

  integer,parameter :: nn=Glob_MaxAllowedNumOfPseudoParticles
  !Arguments:
  integer       p,q
  real(dprec)   inv_tAkl(nn,nn),det_tAkl
  real(dprec)   twk(nn),twl(nn)

  
  !Local variables:
  integer       n,k,i,j
  real(dprec)   XC(nn,nn), CXC(nn,nn)
  real(dprec)   W, Wij, jijCwk, jijCwl 
  real(dprec)   twkinv_tAkl(nn), inv_tAkltwl(nn)
  real(dprec)   commonFactor, gamma, temp, temp1
  real(dprec)   I1, I2
...
function ME_rXr_over_rij_WkWl_real(p,q,X,inv_tAkl,det_tAkl,twk,twl)
  real(dprec)  ME_rXr_over_rij_WkWl_real
  ...
  integer       n,k,i,j
  real(dprec)   XC(nn,nn), CXC(nn,nn)
  real(dprec)   trX, trijX, W, Wij, WX, WijX, WXij, WijXij, &
  jijCwk, jijCwl, jijCXCwl, jijCXCwk, jijCXCjij 
  real(dprec)   twkinv_tAkl(nn), inv_tAkltwl(nn), CXCwl(nn), CXCwk(nn)
  real(dprec)   commonFactor, gamma, temp, temp1
  real(dprec)   I1, I2, I3
...
function ME_over_rij_real(p,q,inv_tAkl,det_tAkl,tvk,tvl,twk,twl)
  ...
  real(dprec)   ME_over_rij_real
  integer,parameter :: nn=Glob_MaxAllowedNumOfPseudoParticles
  !Arguments:
  integer       p, q
  real(dprec)   tAl(nn,nn),tAk(nn,nn),inv_tAkl(nn,nn),det_tAkl
  real(dprec)   tvk(nn),tvl(nn),twk(nn),twl(nn)

  !Local variables
  integer  i,j,k,n 
  real(dprec)   tvkinv_tAkl(nn), twkinv_tAkl(nn), inv_tAkltvl(nn), inv_tAkltwl(nn)
  real(dprec)   V,W,tV,tW,jijCvk,jijCvl,jijCwk,jijCwl,&
  Vij,Wij,tVij,tWij
  real(dprec)  commonFactor, gamma, temp, temp1, temp2, temp3
  real(dprec)  I1,I2,I3
...
function ME_rXr_rYr_over_rij_real(p,q,X,Y,inv_tAkl,det_tAkl,tvk,tvl,twk,twl)
  ...
  real(dprec)   ME_rXr_rYr_over_rij_real
  integer,parameter :: nn=Glob_MaxAllowedNumOfPseudoParticles
  !Arguments:
  ...
  real(dprec) :: tW, tWij, tWX, tWY, tWYX, tWXY, tWijY, tWYij, tWijX, tWXij, &
  ...
  real(dprec) :: jijCvk, jijCvl, jijCwk, jijCwl, &
  ...
  real(dprec) :: trX, trY, trYX, trYij, trXij, trijYX, trYijX, trijYijX
 
  real(dprec) :: I11, I12, I13, I14, I15, I1, &
  I21, I22, I23, I24, I25, I2, &
  I31, I32, I33, I34, I35, I3, &
  I41, I42, I43, I44, I45, I4, &
  I51, I52, I53, I54, I55, I5

  real(dprec) :: CX(nn, nn), XC(nn, nn), CXC(nn, nn), &
  CY(nn, nn), YC(nn, nn), CYC(nn, nn), &
  CXCYC(nn, nn), CYCXC(nn, nn)

  real(dprec) :: CXCvl(nn), CXCwl(nn), CYCvl(nn), CYCwl(nn), &
  ...
  commonFactor = Glob_Piraised3n2/(TWO*SQRTPI*det_tAkl*sqrt(det_tAkl))
...
function ME_rXr_over_rij_real(p,q,Xs,inv_tAkl,det_tAkl,tvk,tvl,twk,twl)
  ...
  real(dprec)   ME_rXr_over_rij_real

  integer,parameter :: nn=Glob_MaxAllowedNumOfPseudoParticles
  !Arguments:
  integer       p,q
  real(dprec)   Xs(nn,nn),inv_tAkl(nn,nn),det_tAkl
  real(dprec)   tvk(nn),tvl(nn),twk(nn),twl(nn)
  ...
  I31, I32, I33, I3, &
  I41, I42, I43, I4
  real(dprec) :: XAl(nn, nn), AXsA(nn, nn), XsA(nn, nn)
  ...
  commonFactor = ONEHALF*Glob_Piraised3n2/(sqrt(PI)*det_tAkl*sqrt(det_tAkl))

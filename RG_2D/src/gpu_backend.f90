module gpu_backend
!The complete GPU backend for RG_2D -- both the high-level orchestration AND the
...
!
!Ported from the RG_0S backend. RG_2D differences handled here:
!  * each basis function carries, besides its vech(L) nonlinear parameters, two
...
!    MatrixElementsHS_RG_2D, which unpacks L and forms A per (pair x term) call
...
!      SHARED MatrixElementsHS_RG_2D device routine in matelem.f90, so the physics
!      lives once, not duplicated), the device lifecycle, and the cuSOLVER
...
!      code cannot read host module globals.
  ...
  use matelem,   only: MatrixElementsHS_RG_2D
  use cudafor
  use cublas
  use cusolverDn
  implicit none
  ...
  integer, parameter :: CUF_BLK = 128       !threads/block. The term loop STRIDES (see the kernels), so
                                            ...
                                            !MatrixElementsHS_RG_2D is register-heavy on device;
                                            !128 threads/block keeps every symmetry-projector size
                                            !launchable (see the RG_0S backend for the full rationale).
...
contains
  ...
  !terms. Each thread calls the shared MatrixElementsHS_RG_2D (grad=false) for
  !its terms.
  attributes(global) subroutine me_energy_kernel(Param, im, imm, np, Kmax, &
    ...
    real(wp) :: dTkl, dVkl               !RG_2D also returns T and V; not needed here
    ...
    if (determ /= 0) then
      ...
      do j = threadIdx%x, nterms, blockDim%x
        if (mod(qbase+j, nprocs) == procid) then
          call MatrixElementsHS_RG_2D(n, np, im(k0), imm(k0), im(l0), imm(l0), &
                              Param(1,k0), Param(1,l0), &
                              YHYMatr((j-1)*n*n+1), mass, chargeM, &
                              sqrtpi, pir3n2, &
                              Hkl, Skl, dTkl, dVkl, dDk, dDl, .false., .false.)
          coeff = YHYCoeff(j)
          shm(j)        = coeff*Hkl
  ...
  subroutine cuf_compute_matelem_batch(Kmax, k_list, l_list, npairs, &
    ...
    call me_energy_kernel<<<npairs, blk, shmem>>>(d_Param, d_im, d_imm, np, Kmax, d_k, d_l, &
  ...
  !Each thread calls the shared MatrixElementsHS_RG_2D (grad_k=true, grad_l=flag)
  !and atomic-accumulates H,S and the gradient slabs Dk,Dl into shared memory.
  attributes(global) subroutine me_grad_kernel(Param, im, imm, np, Kmax, &
    ...
    do j = threadIdx%x, nterms, nthr        !STRIDED: block size independent of nterms
      if (mod(qbase+j, nprocs) == procid) then
        call MatrixElementsHS_RG_2D(n, np, im(k0), imm(k0), im(l0), imm(l0), &
                            ...
                            YHYMatr((j-1)*n*n+1), mass, chargeM, &
                            sqrtpi, pir3n2, &
                            Hkl, Skl, dTkl, dVkl, Dk, Dl, .true., gl)
        coeff = YHYCoeff(j)
        istat = atomicadd(sh_H, coeff*Hkl)
        istat = atomicadd(sh_S, coeff*Skl)
        do q=1,npt2

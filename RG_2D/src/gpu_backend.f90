...
!      code cannot read host module globals.
  ...
  use matelem,   only: MatrixElementsHS_RG_2D, Precompute_LAMA, Precompute_PermutationMaps
  use cudafor
  use cublas
  use cusolverDn
  implicit none
...
contains
  ...
  !Precompute and upload L, A=L*L', mass*A and the prefactor indices ONCE
  ...
  !build shipped the whole basis repeatedly.) The buffers only ever grow.
    ...
    if (Kmax > cap_basis) then
      if (allocated(im))   deallocate(im, imm, Lh, Ah, MAh)
      if (allocated(d_Lm)) deallocate(d_Lm, d_Am, d_MAm, d_im, d_imm)
      allocate(im(Kmax), imm(Kmax), Lh(n,n,Kmax), Ah(n,n,Kmax), MAh(n,n,Kmax))

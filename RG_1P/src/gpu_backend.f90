module gpu_backend
!The complete GPU backend for RG_1P -- both the high-level orchestration AND the
...
!
!Ported from the RG_0S backend. RG_1P differences handled here:
...
!      SHARED MatrixElementsHS_RG_1P device routine in matelem.f90, so the physics
...
!      code cannot read host module globals.
  ...
  integer, parameter :: CUF_BLK = 128       !threads/block. The term loop STRIDES (see the kernels), so
                                            ...
                                            !MatrixElementsHS_RG_1P is register-heavy on device;

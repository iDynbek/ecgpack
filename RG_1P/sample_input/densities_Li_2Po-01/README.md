...
They read files `cf.dat` and `dens.dat`, respectively, and create contour plots in the `png` format for all distributions. Two plots are created for each 2D distribution: the function itself (e.g. $g_{12}(\xi_\rho,\xi_z)$, $\rho_1(\xi_\rho,\xi_z)$) and the corresponding axial probability density (e.g. $2 \pi \xi_\rho g_{12}(\xi_\rho,\xi_z)$, $2 \pi \xi_\rho \rho_1(\xi_\rho,\xi_z)$). The resulting `png` files generated by gnuplot for the considered case of Li atom are provided.  

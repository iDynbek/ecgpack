...
The `RG_0S-1P` code calculates only the **transition matrix elements** (the length-form coordinate matrix element and the velocity-form momentum matrix element). 
...
Run the `RG_0S-1P` executable from the `transition_dipole_moment_He` working directory.
...
Copy and rename the final P-state wave function as wf_state1.txt
             ↓
Run RG_0S-1P in the transition_dipole_moment_He folder
             ↓

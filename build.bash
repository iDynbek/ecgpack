...
usage_print() {
    ...
    echo "" 
    echo "DESCRIPTION:"
    echo "<machinename> is the name of the machine or OS distro currently supported. Only a single value may be specified. It could be wsl_ubuntu_22.04 (default), shabyt, muon."
    ...
    echo "$0 machine=wsl_ubuntu_22.04 toolchain=foss-2023b config=release code=RGL0,RGL1,RGL2P,RGL2D nparticles=3,4,5,6 precision=8,10,16 use_optimized_lapack=no"
...
for nparticles_value in ${nparticles_list[@]}; do
    if ! [[ "$nparticles_values" =~ ^[0-9]+$ && "$nparticles_values" -ge 1 && "$nparticles_value" -le 20 ]]; then
        echo "ERROR, WRONG VALUE(S) OF ARGUMENT: nparticles"
        usage_print
        exit 1
    fi
done
...
if [ "$machine" = "wsl_ubuntu_22.04" ]; then
    # Loop over all toolchains
    for toolchain_value in ${toolchain_list[@]}; do
        # Check if the toolchain is valid for wsl_ubuntu_22.04 and load the corresponding module

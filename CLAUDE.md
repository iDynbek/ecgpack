...
- **`gpu_backend.f90`** — present only in the four energy codes (`RG_0S`, `RG_1P`, `RG_2D`, `RG_2P`), compiled only when `USE_CUDA=yes`. One CUDA Fortran module holding the whole GPU path: device lifecycle, staging of the basis/permutation data, the `attributes(global)` matrix-element kernels, and the cuSOLVER `gpu_dsygvx` eigensolver. The dispatch points are `#ifdef USE_CUDA` guards in `main.f90` (init/finalize), `matform.f90` (`gpu_build_HS`, `gpu_build_HS_deriv`) and `workproc.f90` (`gpu_dsygvx`). Selected at runtime by `ECG_GPU=1` / `ECG_GPU_EIG=1`, so a CUDA-enabled binary is still a normal CPU binary by default. Shabyt targets `sm_70` (V100), while Irgetas targets `sm_90` (H100). Finalization frees application-owned CUDA allocations but must not call `cudaDeviceReset()`: NVHPC 26.5 performs later runtime cleanup and otherwise fails with CUDA error 709.

The GPU path does **not** duplicate the physics. `MatrixElementsHS_*` in `matelem.f90` carries `attributes(host,device)` under `USE_CUDA` and is compiled once for each target from a single body; because device code cannot read host module variables, that routine takes `n`, `np`, the mass/charge matrices and the two π constants as arguments instead of reading the `Glob_*` globals. Anything added to it must therefore stay device-compilable (no host globals, no I/O, no allocatables).

  ...
  do j=1,n  
    ...
    do p=1,n
      do q=1,n
        ...
        W44(p,q) = 2*tAl(p,j)*tbl(i)*tvk(q) + 2*tAl(p,i)*tbl(j)*tvk(q)        
        W4b(p,q) = 2*tAl(p,j)*tbl(i)*tbk(q) + 2*tAl(p,i)*tbl(j)*tbk(q)
        W44b(p,q) = 2*tAl(p,j)*tvl(i)*tbk(q) + 2*tAl(p,i)*tvl(j)*tbk(q)
      enddo
    enddo
    !W4 = W4 + vl vk' Ejj Al Eij
    do p=1,n
      W4(p,j) = W4(p,j) + tvl(p)*tvk(j)*tAl(j,i)
      W44(p,j) = W44(p,j) + tbl(p)*tvk(j)*tAl(j,i)     
      W4b(p,j) = W4b(p,j) + tbl(p)*tbk(j)*tAl(j,i)
      W44b(p,j) = W44b(p,j) + tvl(p)*tbk(j)*tAl(j,i)
    enddo
    do q=1,n
      W4(j,q) = W4(j,q) + 2*tAl(i,j)*tvl(j)*tvk(q) + tr3*tvl(i)*tvk(q) + tvl(i)*tvk(j)*tAl(j,q)     
      W44(j,q) = W44(j,q) + 2*tAl(i,j)*tbl(j)*tvk(q) + tr3*tbl(i)*tvk(q) + tbl(i)*tvk(j)*tAl(j,q)      
      W4b(j,q) = W4b(j,q) + 2*tAl(i,j)*tbl(j)*tbk(q) + tr3*tbl(i)*tbk(q) + tbl(i)*tbk(j)*tAl(j,q)     
      W44b(j,q) = W44b(j,q) + 2*tAl(i,j)*tvl(j)*tbk(q) + tr3*tvl(i)*tbk(q) + tvl(i)*tbk(j)*tAl(j,q)
    enddo

    do p=1,n
      do q=1,n
        W5(p,q) = tAkl(p,j)*tvl(j)*tvk(q) + tAl(p,j)*tvl(j)*tvk(q) + tvl(p)*tvk(j)*tAl(j,q)
        W55(p,q) = tAkl(p,j)*tbl(j)*tvk(q) + tAl(p,j)*tbl(j)*tvk(q) + tbl(p)*tvk(j)*tAl(j,q)       
        W5b(p,q) = tAkl(p,j)*tbl(j)*tbk(q) + tAl(p,j)*tbl(j)*tbk(q) + tbl(p)*tbk(j)*tAl(j,q)
        W55b(p,q) = tAkl(p,j)*tvl(j)*tbk(q) + tAl(p,j)*tvl(j)*tbk(q) + tvl(p)*tbk(j)*tAl(j,q)
      enddo
    enddo
    ...
    !compute integrals   
         ...
         temp44=ONETHIRD*SG_ME_rXr_rYr_over_rij(j,j,W44,tvltbk,inv_tAkl,det_tAkl)
         temp4b=ONETHIRD*SG_ME_rXr_rYr_over_rij(j,j,W4b,tvltvk,inv_tAkl,det_tAkl)
        temp44b=ONETHIRD*SG_ME_rXr_rYr_over_rij(j,j,W44b,tbltvk,inv_tAkl,det_tAkl)    
     ...
     temp55=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W3,W55,tvltbk,inv_tAkl,det_tAkl)  
     temp5b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W3,W5b,tvltvk,inv_tAkl,det_tAkl)
    temp55b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W3,W55b,tbltvk,inv_tAkl,det_tAkl) 
      temp6=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W6,W7,tbltbk,inv_tAkl,det_tAkl)
     temp66=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W6,W77,tvltbk,inv_tAkl,det_tAkl)
     temp6b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W6,W7b,tvltvk,inv_tAkl,det_tAkl)
    temp66b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(j,j,W6,W77b,tbltvk,inv_tAkl,det_tAkl)
...
do i=1,n
  do j=i+1,n
    ...
    do p=1,n
      ...
      W44(p,j) = W44(p,j) + tbl(p)*tvk(j)*tAl(j,i)
      W44(p,i) = W44(p,i) - tbl(p)*tvk(j)*tAl(j,i)      
      W4b(p,j) = W4b(p,j) + tbl(p)*tbk(j)*tAl(j,i)
      W4b(p,i) = W4b(p,i) - tbl(p)*tbk(j)*tAl(j,i)     
      W44b(p,j) = W44b(p,j) + tvl(p)*tbk(j)*tAl(j,i)
      W44b(p,i) = W44b(p,i) - tvl(p)*tbk(j)*tAl(j,i)      
    enddo

    do q=1,n
      temp1 = 2*tAl(i,j)*tvl(j)*tvk(q) + tr3*tvl(i)*tvk(q) + tvl(i)*tvk(j)*tAl(j,q)      
      temp11 = 2*tAl(i,j)*tbl(j)*tvk(q) + tr3*tbl(i)*tvk(q) + tbl(i)*tvk(j)*tAl(j,q)     
      temp1b = 2*tAl(i,j)*tbl(j)*tbk(q) + tr3*tbl(i)*tbk(q) + tbl(i)*tbk(j)*tAl(j,q)
      temp11b = 2*tAl(i,j)*tvl(j)*tbk(q) + tr3*tvl(i)*tbk(q) + tvl(i)*tbk(j)*tAl(j,q)
      W4(j,q) = W4(j,q) + temp1
      W4(i,q) = W4(i,q) - temp1
    ...
    temp2=ME_rXr_rYr_over_rij(i,j,W2,W3,inv_tAkl,det_tAkl,tvk,tvl,tbk,tbl)
      ...
      temp44=ONETHIRD*SG_ME_rXr_rYr_over_rij(i,j,W44,tvltbk,inv_tAkl,det_tAkl)     
      temp4b=ONETHIRD*SG_ME_rXr_rYr_over_rij(i,j,W4b,tvltvk,inv_tAkl,det_tAkl)     
     temp44b=ONETHIRD*SG_ME_rXr_rYr_over_rij(i,j,W44b,tbltvk,inv_tAkl,det_tAkl)      
  ...
  temp55=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W3,W55,tvltbk,inv_tAkl,det_tAkl)     
  temp5b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W3,W5b,tvltvk,inv_tAkl,det_tAkl)
 temp55b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W3,W55b,tbltvk,inv_tAkl,det_tAkl)     
   temp6=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W6,W7,tbltbk,inv_tAkl,det_tAkl)
  temp66=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W6,W77,tvltbk,inv_tAkl,det_tAkl)    
  temp6b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W6,W7b,tvltvk,inv_tAkl,det_tAkl)
 temp66b=ONETHIRD*SG_ME_rXr_rYr_rZr_over_rij(i,j,W6,W77b,tbltvk,inv_tAkl,det_tAkl)
